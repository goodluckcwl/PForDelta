...
//	printf("Starting to compress by using threshold value %f \n", THRESHOLD);
	...
	//memcpy(delta, data, total_nums * sizeof(uint32_t));
...
//			printf("\n and exception number is %d \n", exceptionNums);
	...
	//  got exception value's position info. !!! && force exception !!!
	uint32_t maxOffset = maxValue[min_bits_per_item];
	// enforced exception will add more exceptions into original exception list
	uint32_t maxNumOfEnforcedEpt = total_nums / maxOffset
			+ ((total_nums % maxOffset) ? 1 : 0);
	...
	if (exceptionNums == 0) { // no exception
		...
		uint32_t * exceptions = (uint32_t *) malloc(
				sizeof(uint32_t) * (exceptionNums + maxNumOfEnforcedEpt));
		for (i = 0; i < total_nums; i++) {
			if (least_bits[i] > min_bits_per_item) { // exception
				if (actualEptNums == 0) { // first exception
					exceptions[actualEptNums++] = delta[i]; // put exception into exception list
					cd->exception_offset = i;
				} else {
					if ((i - preException) > maxOffset) {
						// force exception here
						uint32_t numEnforced = (i - preException) / maxOffset;
						uint32_t leftOffset = (i - preException) % maxOffset;
						while (numEnforced) {
							delta[preException] = maxOffset;
							exceptions[actualEptNums++] = delta[preException
									+ maxOffset];
							preException = preException + maxOffset;
							numEnforced--;
						}
						if (leftOffset) {
							delta[preException] = leftOffset;
							exceptions[actualEptNums++] = delta[preException
									+ leftOffset];
						}
					} else {
						exceptions[actualEptNums++] = delta[i]; // put exception into exception list
						delta[preException] = i - preException;
					}
				}
//				printf("delta[%u],  position [%u], exceptions : %u \n", delta[preException], i, exceptions[actualEptNums - 1]);
				preException = i;
			}
		}
		// WARNING: we need this one for the write_to_bitstream
		delta[preException] = 0;
...
//	printf("a[%u],\n",a);
	...
	uint32_t next_exception = cd->exception_offset;
	uint32_t exception_num = cd->exception_nums;
	uint32_t unpackedIndex = next_exception;
	uint32_t fetch = 0;
	while (fetch < exception_num) {

		next_exception = dcd->unpacked[unpackedIndex];
		dcd->unpacked[unpackedIndex] = cd->exceptions[fetch++]; // fetch exceptions
		unpackedIndex += next_exception;
		/*printf(
				"index[%u], exception value %u , next exeption's offset [%u] & position[%u] \n ", fetch -1,
				dcd->unpacked[unpackedIndex - next_exception], next_exception, unpackedIndex);*/
	}
	uint32_t i;
